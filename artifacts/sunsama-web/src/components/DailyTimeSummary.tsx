...
export function DailyTimeSummary({ tasks }: DailyTimeSummaryProps) {
  ...
  const unscheduledMinutes = unscheduledTasks.reduce(
    (sum, task) => sum + (task.timeEstimate || 0),
    0
  );
      ...
      <div className="space-y-2 text-sm">
        ...
        <div className="flex justify-between">
          ...
          <span className="font-semibold text-green-700">
            {scheduledMinutes > 0
              ? formatTime(scheduledHours, scheduledMins)
              : "0m"}
          </span>
        </div>
        ...
        <div className="flex justify-between">
          ...
          <span className="font-semibold text-orange-600">
            {unscheduledMinutes > 0
              ? formatTime(unscheduledHours, unscheduledMins)
              : "0m"}
          </span>
        </div>
        ...
        <div className="flex justify-between">
          ...
          <span className="font-semibold text-blue-900">
            {tasks.length} tasks ({tasksWithEstimates} estimated,{" "}
            {tasksWithoutEstimates} not estimated)
          </span>
        </div>
        ...
        <div className="flex justify-between">
          ...
          <span className="font-semibold text-blue-900">
            {Math.round((totalMinutes / availableMinutes) * 100)}% of your day
            planned
          </span>
        </div>
      </div>
